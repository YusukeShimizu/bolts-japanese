...
この文書では、署名を有効にするために双方が合意しなければならないオンチェーントランザクションの正確な形式を定めます。対象は、資金調達トランザクションの出力スクリプト、コミットメントトランザクション、および HTLC トランザクションです。
...
# 目次
  ...
  * [トランザクション](#transactions)
    ...
    * [従来のクローズトランザクション](#legacy-closing-transaction)
  ...
  * [鍵](#keys)
    * [鍵の導出](#key-derivation)
        * [`localpubkey`, `local_htlcpubkey`, `remote_htlcpubkey`, `local_delayedpubkey`, および `remote_delayedpubkey` の導出](#localpubkey-local_htlcpubkey-remote_htlcpubkey-local_delayedpubkey-and-remote_delayedpubkey-derivation)
        * [`remotepubkey` の導出](#remotepubkey-derivation)
  ...
  * [付録 A: 期待されるウェイト](#appendix-a-expected-weights)
      * [資金調達トランザクションの期待されるウェイト (v2 チャネル確立)](#expected-weight-of-the-funding-transaction-v2-channel-establishment)
      * [コミットメントトランザクションの期待されるウェイト](#expected-weight-of-the-commitment-transaction)
      * [HTLC-timeout と HTLC-success トランザクションの期待されるウェイト](#expected-weight-of-htlc-timeout-and-htlc-success-transactions)
  ...
  * [付録 G: デュアルファンディングトランザクションのテストベクトル](#appendix-g-dual-funded-transaction-test-vectors)
  * [参考文献](#references)
  * [著者](#authors)
...
トランザクション内の出力は、常に以下の順序でソートされます:
 * まず金額の昇順（小さい方から先。整数サトシ単位で比較し、HTLC 出力では millisatoshi 部分を無視します）
 * 次に `scriptpubkey` を `memcmp` 相当で辞書順比較し、共通プレフィックスが等しい場合は短いスクリプトを優先します
 * 最後に、HTLC 出力の場合は `cltv_expiry` の昇順で並べます

## 理論的根拠

同じ `amount`（`amount_msat` を丸めた値）と `payment_hash` を持つ 2 つの提供 HTLC は、`cltv_expiry` が異なっていても同一の出力を持ちます。`htlc_signatures` を送信する際にも同じ順序が使われ、HTLC トランザクション自体は異なるため、このケースで両ピアが同一の正規順序に合意する必要があるという点でのみ重要になります。

## Segwit の使用

ここで使われるほとんどのトランザクション出力は、pay-to-witness-script-hash<sup>[BIP141](https://github.com/bitcoin/bips/blob/master/bip-0141.mediawiki#witness-program)</sup> (P2WSH) 出力、つまり Segwit 版の P2SH です。このような出力を消費するには、witness スタックの最後の項目が、消費対象の P2WSH 出力を生成した実際のスクリプトでなければなりません。本文書の残りの部分では、簡潔さのためにこの最後の項目を省略しています。
...
* 資金調達出力のスクリプトは、次の値への P2WSH です:

`2 <pubkey1> <pubkey2> 2 OP_CHECKMULTISIG`

* ここで `pubkey1` は圧縮形式の 2 つの `funding_pubkey` のうち辞書順で小さい方、`pubkey2` は辞書順で大きい方です。
...
48 ビットのコミットメント番号は、以下の値の下位 48 ビットと `XOR` することで難読化されます。

    SHA256(payment_basepoint from open_channel || payment_basepoint from accept_channel)

これにより、片側からの一方的クローズが行われた場合でもチャネル上で作成されたコミットメントの数を隠せます。一方で、`payment_basepoint` を知る両ノードにとっては、取り消されたコミットメントトランザクションを素早く特定するための有用なインデックスとして機能します。

### コミットメントトランザクションの出力

取り消されたコミットメントトランザクションが公開された場合に備えてペナルティトランザクションを行う機会を確保するため、コミットメントトランザクションの所有者（いわゆる「ローカルノード」）へ資金を返すすべての出力は、`to_self_delay` ブロックの間遅延されなければなりません。HTLC については、この遅延は第二段階の HTLC トランザクション（ローカルノードが受け取った HTLC については HTLC-success、ローカルノードが提供した HTLC については HTLC-timeout）で実現されます。

HTLC 出力に対して別のトランザクション段階を設けるのは、HTLC が `to_self_delay` の遅延期間内であってもタイムアウトしたり履行されたりできるようにするためです。さもなければ、HTLC に課される最小タイムアウトがこの遅延の分だけ長くなり、ネットワーク全体を経由する HTLC のタイムアウトが長くなってしまいます。

各出力の金額は整数サトシ単位に切り捨てなければなりません。この金額から HTLC トランザクションの手数料を引いた値が、コミットメントトランザクション所有者の設定する `dust_limit_satoshis` を下回る場合、その出力は生成してはなりません（その分は手数料に加算されます）。

#### `to_local` 出力

この出力は、コミットメントトランザクションの所有者に資金を戻すものなので、`OP_CHECKSEQUENCEVERIFY` でタイムロックされなければなりません。取り消し秘密鍵を知っている相手側は、遅延なしでこの出力を請求できます。出力はバージョン 0 の P2WSH で、witness スクリプトは次のとおりです。
...
この出力は、`nSequence` フィールドを `to_self_delay` に設定した入力と次の witness によって消費されます（このシーケンス値は `to_self_delay` ブロックが経過したあとでのみ有効です）。

    <local_delayedsig> <>

取り消されたコミットメントトランザクションが公開された場合、相手側は次の witness によってこの出力を即座に消費できます。

    <revocation_sig> 1

#### `to_remote` 出力

コミットメントトランザクションに `option_anchors` が適用される場合、`to_remote` 出力は 1 ブロックの CSV ロックで制約されます。

    <remotepubkey> OP_CHECKSIGVERIFY 1 OP_CHECKSEQUENCEVERIFY

この出力は、`nSequence` フィールドを `1` に設定した入力と次の witness で消費されます。

    <remote_sig>

それ以外の場合、この出力は `remotepubkey` への単純な P2WPKH です。

#### `to_local_anchor` と `to_remote_anchor` 出力 (option_anchors)

この出力は、child-pays-for-parent (CPFP) によりトランザクションのマイニングを促すインセンティブとして、ローカルノードとリモートノードがそれぞれ消費できる出力です。アンカー出力は常に両方追加されますが、HTLC が存在せず、かつ片方の当事者のコミットメント出力がダスト制限を下回る場合だけは例外で、その場合は実際に生成されるコミットメント出力に対応するアンカーのみが追加されます。これは通常、イニシエータがチャネルを開いた直後にクローズする場合（`to_remote` 出力が存在しない場合）に発生します。
...
各当事者は自身の `funding_pubkey` にロックされた独自のアンカー出力を持ちます。これは、悪意のあるピアが低い手数料密度の子トランザクションをアンカーに付加し、被害者のコミットメントトランザクションが期限内に確認されるのを妨げる攻撃を防ぐためです。この防御は Bitcoin Core 0.19 の変更で実現可能になりました: https://github.com/bitcoin/bitcoin/pull/15681 。コミットメントトランザクション上のすべての非アンカー出力が CSV ロックされているのも同じ理由によります。

UTXO セットの汚染を防ぐため、コミットメントトランザクションの確認後に未消費のまま残ったアンカーは、誰でも消費できるようになります。これが、アンカー出力を `funding_pubkey` にロックする理由でもあります。第三者はこの鍵を観察すれば、コミットメント出力が一切消費されなくても支出スクリプトを再構築できます。ただしこれは、いずれは手数料市場が下がってアンカーをスイープすることが採算に合うレベルになることを前提としています。

出力の金額は P2WSH のデフォルトダスト制限である 330 サトシに固定されます。

この出力を消費するには、次の witness を使います。

    <local_sig/remote_sig>

16 ブロック経過後は、誰でも次の witness でアンカーをスイープできます。

    <>

#### 提供された HTLC 出力

この出力は、HTLC のタイムアウト後に HTLC-timeout トランザクションへ資金を送るか、もしくは支払いプリイメージまたは取り消し鍵を使ってリモートノードへ資金を送ります。出力は P2WSH で、witness スクリプトは以下のとおりです（`option_anchors` がない場合）。
    ...
    OP_ELSE
        ...
        OP_NOTIF
            # HTLC-timeout トランザクション経由でローカルノードへ（タイムロックあり）
...
リモートノードは、次の witness を用いて HTLC を引き出せます。

    <remotehtlcsig> <payment_preimage>

`option_anchors` が適用される場合、消費する入力の nSequence フィールドは `1` でなければならない点に注意してください。

取り消されたコミットメントトランザクションが公開された場合、リモートノードは次の witness を用いてこの出力を即座に消費できます。

    <revocation_sig> <revocationpubkey>

送信ノードは、HTLC が期限切れになった後、後述の HTLC-timeout トランザクションを使って HTLC をタイムアウトさせることができます。これがローカルノードが HTLC をタイムアウトできる唯一の方法です。この分岐には `<remotehtlcsig>` が必要で、HTLC-timeout トランザクションは `cltv_expiry` を `locktime` に持つため、ローカルノードが HTLC を早すぎるタイミングでタイムアウトさせることはできません。さらにローカルノードは資金へアクセスする前に `to_self_delay` を待たなければならず、もしこのトランザクションが取り消されていればリモートノードが代わりに資金を請求できます。

#### 受信 HTLC 出力

この出力は、HTLC のタイムアウト後または取り消し鍵によってリモートノードへ資金を送るか、または有効な支払いプリイメージを伴う HTLC-success トランザクションへ資金を送ります。出力は P2WSH で、witness スクリプトは以下のとおりです（`option_anchors` がない場合）。
    ...
    OP_ELSE
        <remote_htlcpubkey> OP_SWAP OP_SIZE 32 OP_EQUAL
        OP_IF
            # HTLC-success トランザクション経由でローカルノードへ
            OP_HASH160 <RIPEMD160(payment_hash)> OP_EQUALVERIFY
...
リモートノードは、次の witness で HTLC をタイムアウトさせて消費します。

    <remotehtlcsig> <>

`option_anchors` が適用される場合、消費する入力の nSequence フィールドは `1` でなければなりません。

取り消されたコミットメントトランザクションが公開された場合、リモートノードは次の witness でこの出力を即座に消費できます。

    <revocation_sig> <revocationpubkey>

HTLC を引き出すには、後述する HTLC-success トランザクションを使用します。これがローカルノードが HTLC を消費できる唯一の方法です。この分岐には `<remotehtlcsig>` が必要であり、ローカルノードは資金へアクセスする前に `to_self_delay` を待たなければなりません。これにより、もしこのトランザクションが取り消されていれば、リモートノードが代わりに資金を請求できます。

### トリムされた出力

各ピアは `dust_limit_satoshis` を指定します。この値を下回る出力は生成すべきではなく、生成されないこれらの出力は「トリムされた」と呼ばれます。トリムされた出力は生成するに値しないほど小さいと見なされ、代わりにコミットメントトランザクションの手数料に加算されます。

HTLC については、第二段階の HTLC トランザクションもこの制限を下回り得ることを考慮する必要があります。`option_anchors` を使用する場合、HTLC トランザクションは手数料を含まないためトリミングには寄与しません。そのため、これらのチャネルでは、出力を経済的に消費可能にするためにより高い `dust_limit_satoshis` を設定するのが理にかなっています。

#### 要件

基本手数料とアンカー出力の金額:
  - コミットメントトランザクションの出力が確定する前に:
    - [手数料計算](#fee-calculation) に従って `to_local` または `to_remote` 出力から差し引かなければならない。

コミットメントトランザクション:
  - `to_local` 出力の金額がトランザクション所有者の `dust_limit_satoshis` を下回る場合:
    - その出力を含めてはならない。
  - そうでない場合:
    - [`to_local` 出力](#to_local-output) の指定どおりに生成しなければならない。
  - `to_remote` 出力の金額がトランザクション所有者の `dust_limit_satoshis` を下回る場合:
    - その出力を含めてはならない。
  - そうでない場合:
    - [`to_remote` 出力](#to_remote-output) の指定どおりに生成しなければならない。
  - 提供された各 HTLC について:
    - HTLC 金額から HTLC-timeout 手数料を引いた値がトランザクション所有者の `dust_limit_satoshis` を下回る場合:
      - その出力を含めてはならない。
    - そうでない場合:
      - [提供された HTLC 出力](#offered-htlc-outputs) の指定どおりに生成しなければならない。
  - 受信した各 HTLC について:
    - HTLC 金額から HTLC-success 手数料を引いた値がトランザクション所有者の `dust_limit_satoshis` を下回る場合:
      - その出力を含めてはならない。
    - そうでない場合:
      - [受信 HTLC 出力](#received-htlc-outputs) の指定どおりに生成しなければならない。

## HTLC-Timeout および HTLC-Success トランザクション

これらの HTLC トランザクションはほぼ同一ですが、HTLC-timeout トランザクションはタイムロックされている点が異なります。HTLC-timeout / HTLC-success のいずれも、有効なペナルティトランザクションによって消費される可能性があります。
...
ペナルティとして消費する場合、リモートノードは witness stack `<revocationsig> 1` を用います。出力を回収する場合、ローカルノードは nSequence を `to_self_delay` に設定した入力と witness stack `<local_delayedsig> 0` を用います。

## 従来のクローズトランザクション

このバリアントは `closing_signed` メッセージで使用されます（つまり、`option_simple_close` がネゴシエートされない場合）。
...
* txin count: 1
   ...
   * `txin[0]` witness: `0 <signature_for_pubkey1> <signature_for_pubkey2>`
* txout count: 1 または 2
   * `txout` amount: 一方のノードに支払われる最終残高（このピアがチャネルに資金を提供した場合は `closing_signed` の `fee_satoshis` を差し引く）
   * `txout` script: そのノードの `shutdown` メッセージで指定された `scriptpubkey`

### 要件

署名を提供する各ノードは:
  - 各出力をサトシ単位に切り捨てなければならない。
  - `fee_satoshis` で指定された手数料を資金提供者への出力から差し引かなければならない。
  - 自身の `dust_limit_satoshis` 未満の出力を取り除かなければならない。
  - 自身の出力を削除してよい。

### 理論的根拠

一方のノードが相手側の出力をビットコインネットワーク上で伝播させるには小さすぎる（いわゆる「ダスト」）と判断し、もう一方はその出力を捨てるには惜しいと判断した場合、クローズ時に修復不能な不一致が生じる可能性があります。これが、双方が自身の `dust_limit_satoshis` を使用する理由であり、クローズトランザクションの形について意見が分かれた場合には署名検証が失敗することがあります。

一方で、片側が自身の出力を削除することを選んだ場合に、もう一方がクローズプロトコルを失敗させる理由はありません。そのためこの選択は明示的に許可されており、署名がどのバリアントを使用したかを示します。

資金額が `dust_limit_satoshis` の 2 倍を超える場合は、少なくとも 1 つの出力が存在します。

## クローズトランザクション

このバリアントは `closing_complete` および `closing_sig` メッセージで使用されます（つまり、`option_simple_close` がネゴシエートされる場合）。

この場合、`closing_complete` を送信するノード（「クローザー」）が手数料を支払います。出力の順序は [トランザクション出力の順序](#transaction-output-ordering) に従います。

資金が少ない側は、自身の出力を省略することを選択できます。

* version: 2
* locktime: `closing_complete` メッセージの `locktime`
* txin count: 1
   * `txin[0]` outpoint: チャネル出力の `txid` と `output_index`
...
* txout count: 1 または 2
  * クローザー出力:
    * `txout` amount:
      * `scriptpubkey` が `OP_RETURN` で始まる場合は 0
      * それ以外はクローザーの最終残高から `closing_complete.fee_satoshis` を引いた値（サトシ単位に切り捨て）
	* `txout` script: `closing_complete` メッセージの `closer_scriptpubkey` で指定されたもの
  * クローシー（相手側）出力:
    * `txout` amount:
      * `scriptpubkey` が `OP_RETURN` で始まる場合は 0
      * それ以外はクローシーの最終残高（サトシ単位に切り捨て）
	* `txout` script: `closing_complete` メッセージの `closee_scriptpubkey` で指定されたもの

### 要件

署名を提供する各ノードは:
  - 各出力をサトシ単位に切り捨てなければならない。
  - `fee_satoshis` で指定された手数料をクローザー出力から差し引かなければならない。
  - `scriptpubkey` が `OP_RETURN` の場合は出力金額を 0 に設定しなければならない。

## 手数料

### 手数料の計算

コミットメントトランザクションと HTLC トランザクションの手数料計算は、現在の `feerate_per_kw` とトランザクションの *期待されるウェイト* に基づきます。

実際のウェイトと期待されるウェイトは、以下の理由で食い違うことがあります。

* Bitcoin は DER エンコードされた署名を使うためサイズが変動する。
* Bitcoin は可変長整数も使うため、出力数が多くなるとエンコードに 1 バイトではなく 3 バイトを要することがある。
* `to_remote` 出力がダスト制限を下回ることがある。
* 手数料を差し引いた結果、`to_local` 出力がダスト制限を下回ることがある。

そのため、*期待されるウェイト* には次の前提を置いた簡略式を使います。

* 署名は 73 バイト（最大長）。
* 出力数は少ない（したがってカウントは 1 バイト）。
* 常に `to_local` 出力と `to_remote` 出力の両方が存在する。
* （`option_anchors` がある場合）常に `to_local_anchor` と `to_remote_anchor` の両方が存在する。

これにより、以下の *期待されるウェイト* が得られます（計算の詳細は [付録 A](#appendix-a-expected-weights) を参照）。

    コミットメントウェイト (option_anchors なし):   724 + 172 * num-untrimmed-htlc-outputs
    コミットメントウェイト (option_anchors あり):  1124 + 172 * num-untrimmed-htlc-outputs
    HTLC-timeout ウェイト (option_anchors なし): 663
    HTLC-timeout ウェイト (option_anchors あり): 666
    HTLC-success ウェイト (option_anchors なし): 703
    HTLC-success ウェイト (option_anchors あり): 706

以下の要件で言及されるコミットメントトランザクションの「基本手数料」は、資金提供者が支払うものです。実際の手数料は、丸めやトリムされた出力の影響により、ここで計算された値より高くなる場合があります。

#### 要件

HTLC-timeout トランザクションの手数料:
  - `option_anchors` が適用される場合:
    1. 0 でなければならない。
  - それ以外の場合は以下と一致するように計算しなければならない:
    1. `feerate_per_kw` に 663 を掛け、1000 で割る（切り捨て）。

HTLC-success トランザクションの手数料:
  - `option_anchors` が適用される場合:
    1. 0 でなければならない。
  - それ以外の場合は以下と一致するように計算しなければならない:
    1. `feerate_per_kw` に 703 を掛け、1000 で割る（切り捨て）。

コミットメントトランザクションの基本手数料:
  - 以下と一致するように計算しなければならない:
    1. `weight` を 724（`option_anchors` が適用される場合は 1124）で初期化する。
    2. コミットされた各 HTLC について、その出力が [トリムされた出力](#trimmed-outputs) でトリミングされない場合、`weight` に 172 を加算する。
    3. `feerate_per_kw` に `weight` を掛け、1000 で割る（切り捨て）。

#### 例

例として、`feerate_per_kw` が 5000、`dust_limit_satoshis` が 546 サトシで、コミットメントトランザクションが次の HTLC を含む場合を考えます。
* 5,000,000 ミリサトシ（5000 サトシ）と 1,000,000 ミリサトシ（1000 サトシ）の提供 HTLC が 2 件
* 7,000,000 ミリサトシ（7000 サトシ）と 800,000 ミリサトシ（800 サトシ）の受信 HTLC が 2 件

HTLC-timeout トランザクションの `weight` は 663 なので、手数料は 3315 サトシです。
HTLC-success トランザクションの `weight` は 703 なので、手数料は 3515 サトシです。

コミットメントトランザクションの `weight` は次のように計算されます。

* `weight` は 724 から始まります。

* 5000 サトシの提供 HTLC は 546 + 3315 を上回るため、次のようになります。
  * コミットメントトランザクションに 5000 サトシの出力が追加される
  * この出力を消費する 5000 - 3315 サトシの HTLC-timeout トランザクションができる
  * `weight` は 896 に増える

* 1000 サトシの提供 HTLC は 546 + 3315 を下回るためトリムされます。

* 7000 サトシの受信 HTLC は 546 + 3515 を上回るため、次のようになります。
  * コミットメントトランザクションに 7000 サトシの出力が追加される
  * この出力を消費する 7000 - 3515 サトシの HTLC-success トランザクションができる
  * `weight` は 1068 に増える

* 800 サトシの受信 HTLC は 546 + 3515 を下回るためトリムされます。

基本コミットメントトランザクション手数料は 5340 サトシです。実際の手数料（ダストとなる 1000 サトシと 800 サトシの HTLC が加算されるため）は 7140 サトシになります。さらに `to_local` または `to_remote` 出力が `dust_limit_satoshis` を下回ると、最終的な手数料はもっと高くなることがあります。

### 手数料の支払い

基本コミットメントトランザクション手数料と、`to_local_anchor` および `to_remote_anchor` 出力の金額は、資金提供者の金額から差し引かれます。資金提供者のコミットメントトランザクション出力に対するチャネルリザーブ関連の制約は、[BOLT #2](02-peer-protocol.md) に記載されたとおりに適用されます。

なお、手数料を資金提供者の出力から差し引いた結果、その出力が `dust_limit_satoshis` を下回ることもあり、その場合はその出力分も手数料に加算されます。
...
[インタラクティブプロトコル](02-peer-protocol.md#interactive-transaction-construction) で構築されるトランザクションについては、手数料は開始時に決定された `feerate` に基づき各当事者が支払い、共通のトランザクションフィールド分の手数料はイニシエータが負担します。
...
- pay to pubkey hash (p2pkh): 546 サトシ
- pay to script hash (p2sh): 540 サトシ
- pay to witness pubkey hash (p2wpkh): 294 サトシ
- pay to witness script hash (p2wsh): 330 サトシ
- pay to anchor (p2a): 240 サトシ
- 不明な segwit バージョン: 354 サトシ
- `OP_RETURN` 出力: ダストとは見なされません

この計算の理論的根拠（[こちら](https://github.com/bitcoin/bitcoin/blob/2aff9a36c352640a263e8b5de469710f7e80eb54/src/policy/policy.cpp#L28) で実装されています）は、以下のセクションで説明します。

これらのセクションでは、Bitcoin Core の実装に従い 3000 sat/kB の手数料レートで計算します。

なお、`option_anchors` の導入後は、出力をコミットメントトランザクションに含めるかどうかを判断する際に第二段階の HTLC トランザクションのウェイトは考慮されなくなりました。そのため、これらの第二段階 HTLC トランザクションのコストを考慮した `dust_limit_satoshis` を使うことに意味があり、コミットメントトランザクションに追加された出力が実際にオンチェーンで請求できることを保証できます。さもなければ UTXO セットを永続的に汚染する可能性があります。少なくともノードは、ピアが Bitcoin Core が定めた値より高い `dust_limit_satoshis` を使用することを許容すべきです。将来の手数料レートは予測できないため、これは常に機能するとは限らず、オンチェーン手数料が高くなりすぎれば結局使えない HTLC 出力が生じることもあります。一方で、`dust_limit_satoshis` を非常に大きな値にもできません。コミットメントトランザクションに大きすぎるダストエクスポージャーを生じさせるためです（詳細は [こちら](/02-peer-protocol.md#bounding-exposure-to-trimmed-in-flight-htlcs-max_dust_htlc_exposure_msat)）。
...
- シーケンスに 4 バイト
- スクリプトシグの長さに 1 バイト
- witness に 26 バイト（26.75 を切り捨て、75% の segwit 割引を適用）:
  - アイテム数に 1 バイト
  - 署名の長さに 1 バイト
  - 署名に 71 バイト
  - 公開鍵の長さに 1 バイト
...
p2wsh の入力は、基礎となるスクリプトに依存するため固定サイズではありません。そのため、67 バイトを下限として使用します。

p2wsh のダスト閾値は次のようになります：`(43 + 67) * 3000 / 1000 = 330 satoshis`

### 不明な segwit バージョン

不明な segwit 出力は最大で 51 バイトです：

- 出力量に 8 バイト
- スクリプト長に 1 バイト
- スクリプトに 42 バイト（`OP_1` から `OP_16` までを含み、2 から 40 バイトの単一プッシュが続く）

入力は基礎となるスクリプトに依存するため固定サイズではありません。そのため、67 バイトを下限として使用します。

不明な segwit バージョンのダスト閾値は次のようになります：`(51 + 67) * 3000 / 1000 = 354 satoshis`

## コミットメントトランザクションの構築

このセクションでは、これまでの内容を統合し、あるピアのコミットメントトランザクションを構築するアルゴリズムを示します。前提として、そのピアの `dust_limit_satoshis`、現在の `feerate_per_kw`、各ピアへ支払うべき金額 (`to_local` と `to_remote`)、コミットされたすべての HTLC が与えられているものとします。

1. [コミットメントトランザクション](#commitment-transaction) で定義されたとおり、コミットメントトランザクションの入力とロックタイムを初期化する。
2. コミットされた HTLC のうちトリムする必要があるものを判定する（[トリムされた出力](#trimmed-outputs) を参照）。
3. 基本 [コミットメントトランザクション手数料](#fee-calculation) を計算する。
4. この基本手数料を資金提供者（`to_local` または `to_remote`）から差し引く。`option_anchors` が適用される場合は、固定アンカーサイズ 330 サトシの 2 倍も追加で資金提供者から差し引く。
5. 提供されたすべての HTLC について、トリムされていなければ [提供された HTLC 出力](#offered-htlc-outputs) を追加する。
6. 受信したすべての HTLC について、トリムされていなければ [受信 HTLC 出力](#received-htlc-outputs) を追加する。
7. `to_local` の金額が `dust_limit_satoshis` 以上であれば、[`to_local` 出力](#to_local-output) を追加する。
8. `to_remote` の金額が `dust_limit_satoshis` 以上であれば、[`to_remote` 出力](#to_remote-output) を追加する。
9. `option_anchors` が適用される場合:
   * `to_local` が存在する、またはトリムされていない HTLC がある場合は [`to_local_anchor` 出力](#to_local_anchor-and-to_remote_anchor-output-option_anchors) を追加する。
   * `to_remote` が存在する、またはトリムされていない HTLC がある場合は [`to_remote_anchor` 出力](#to_local_anchor-and-to_remote_anchor-output-option_anchors) を追加する。
10. 出力を [BIP 69+CLTV 順序](#transaction-output-ordering) に並べ替える。

# 鍵

## 鍵の導出

各コミットメントトランザクションは一意の `localpubkey` を使います。HTLC-success トランザクションと HTLC-timeout トランザクションは `local_delayedpubkey` と `revocationpubkey` を使い、これらの鍵は `per_commitment_point` に基づきトランザクションごとに変化します。

鍵を毎回変える目的は、取り消されたトランザクションの監視をトラストレスに外部委託できるようにするためです。このような _ウォッチャー_ には、たとえ監視対象のトランザクション ID を知っており、どの HTLC や残高が含まれそうかを合理的に推測できたとしても、コミットメントトランザクションの内容そのものを特定できないようにする必要があります。それでもなお、コミットメントトランザクションをすべて保存させずに済ませるため、_ウォッチャー_ には `per_commitment_secret` の値（コンパクトに保存できます）と、ペナルティトランザクションに必要なスクリプトを再生成するための `revocation_basepoint` および `delayed_payment_basepoint` を渡せます。よって _ウォッチャー_ は、各ペナルティ入力の署名だけを受け取って保存すればよいことになります。

`localpubkey` を毎回変えることで、`to_local` 出力が存在しないという自明な場合を除き、コミットメントトランザクション ID は推測できなくなります（すべてのコミットメントトランザクションは出力スクリプトの中に ID を埋め込んでいるため）。ペナルティトランザクションに必要な `local_delayedpubkey` を分離することで、`localpubkey` を明かさずに _ウォッチャー_ と共有でき、たとえ両ピアが同じ _ウォッチャー_ を使っていても情報は漏れません。

最後に、通常の一方的なクローズの場合でも、HTLC-success や HTLC-timeout トランザクションが _ウォッチャー_ に何かを開示することはありません。_ウォッチャー_ は対応する `per_commitment_secret` を知らず、`local_delayedpubkey` や `revocationpubkey` を基点と結びつけることができないからです。

効率のため、鍵は単一のシードから生成される per-commitment secret 列から導出されます。これにより受信者はコンパクトに保存できます（[下記](#efficient-per-commitment-secret-storage) を参照）。

### `localpubkey`、`local_htlcpubkey`、`remote_htlcpubkey`、`local_delayedpubkey`、および `remote_delayedpubkey` の導出

これらの公開鍵は、基点に加算するだけで導出されます。

	pubkey = basepoint + SHA256(per_commitment_point || basepoint) * G

- `localpubkey` はローカルノードの `payment_basepoint` を使う。
- `local_htlcpubkey` はローカルノードの `htlc_basepoint` を使う。
- `remote_htlcpubkey` はリモートノードの `htlc_basepoint` を使う。
- `local_delayedpubkey` はローカルノードの `delayed_payment_basepoint` を使う。
- `remote_delayedpubkey` はリモートノードの `delayed_payment_basepoint` を使う。

対応する秘密鍵も、基点の秘密が分かっていれば同様に導出できます（つまり、`localpubkey`、`local_htlcpubkey`、`local_delayedpubkey` に対応する秘密鍵のみ）。

    privkey = basepoint_secret + SHA256(per_commitment_point || basepoint)

### `remotepubkey` の導出

`remotepubkey` は単にリモートノードの `payment_basepoint` そのものです。

これは、ノードがデータを失って対応する `per_commitment_point` を知らない状態でもコミットメントトランザクションを消費できることを意味します。ウォッチタワーは、`to_remote` 出力しか持たないトランザクションがオンチェーンに現れた場合に、自身が受け取っているトランザクション情報と紐づけられる可能性があります。しかしそのようなトランザクションには強制執行は不要なので、そもそもウォッチタワーには渡すべきではありません。

### `revocationpubkey` の導出

`revocationpubkey` はブラインド鍵です。ローカルノードがリモートノード向けの新しいコミットメントを作成する際、自身の `revocation_basepoint` とリモートノードの `per_commitment_point` を使ってそのコミットメント用の `revocationpubkey` を導出します。後にリモートノードが使用済みの `per_commitment_secret` を公開する（これによりそのコミットメントが取り消される）と、ローカルノードは鍵導出に必要な 2 つの秘密（`revocation_basepoint_secret` と `per_commitment_secret`）の両方を知ることになり、`revocationprivkey` を導出できます。

`per_commitment_point` は楕円曲線スカラー倍算により生成されます。

	per_commitment_point = per_commitment_secret * G

これはリモートノードの `revocation_basepoint` と組み合わせて revocation 公開鍵を導出するために使われます。

	revocationpubkey = revocation_basepoint * SHA256(revocation_basepoint || per_commitment_point) + per_commitment_point * SHA256(per_commitment_point || revocation_basepoint)

この構成により、basepoint を提供するノードも `per_commitment_point` を提供するノードも、相手側の秘密なしには秘密鍵を導出できないことが保証されます。

対応する秘密鍵は、`per_commitment_secret` が判明すれば導出できます。
...
per-commitment secret は最大 (2^48 - 1) 個まで生成できます。

最初に使うシークレット:
  - インデックスは 281474976710655 でなければならない。
    - そこからインデックスを 1 ずつ減らしていく。

I 番目のシークレット P:
  - 次のアルゴリズムの出力と一致しなければならない:
```
...
ここで `flip(B)` は、値の (B div 8) バイト目の (B mod 8) 番目のビットを反転させます。たとえば `e3b0...` に対する `flip(0)` は `e2b0...`、`e3b0...` に対する `flip(10)` は `e3b4...` です。

受信ノードは:
  - これまでの per-commitment secret をすべて保存してよい。
  - 以下に示すコンパクトな表現から計算してもよい。

## 効率的な Per-commitment Secret の保存

一連のシークレットの受信者は、49 個の (value, index) ペアからなる配列にコンパクトに保存できます。なぜなら 2^X 境界上のあるシークレットがあれば、次の 2^X 境界までのすべてのシークレットを導出可能だからです。シークレットは常に `0xFFFFFFFFFFFF` から始まる降順で受信されます。

二進表現では、任意のインデックスを *プレフィックス* と、それに続く末尾の 0 列として捉えると便利です。あるシークレットがあれば、その *プレフィックス* に一致する任意のインデックスのシークレットを導出できます。

たとえばシークレット `0xFFFFFFFFFFF0` からは `0xFFFFFFFFFFF1` から `0xFFFFFFFFFFFF` までのシークレットを導出でき、`0xFFFFFFFFFF08` からは `0xFFFFFFFFFF09` から `0xFFFFFFFFFF0F` までのシークレットを導出できます。

これは、先ほどの `generate_from_seed` をわずかに一般化した次の関数を使って実現します。
...
各ユニークなプレフィックスに対して 1 つのシークレットだけを保存すればよく、実装上は末尾の 0 の数を数えて、その値からストレージ配列内の保存位置を決定します。
...
それまでに受け取ったすべてのシークレットが正しく導出できるかを確認するダブルチェックが必要です。このチェックが失敗した場合、シークレットは同じシードから生成されていないことを意味します。
...
最後に、インデックス `I` の未知のシークレットを導出する必要がある場合、どの既知のシークレットから導出できるかを見つけなければなりません。最も単純な方法は、既知のシークレットを順に試し、それぞれを使って目的のシークレットを導出できるかを確認することです。
...
これは複雑に見えますが、エントリ `b` のインデックスは末尾に `b` 個の 0 を持つという点を思い出してください。マスクと比較は、各バケットのインデックスが目的のインデックスのプレフィックスかどうかを単純に確認しているだけです。

# Appendix A: Expected Weights

## Expected Weight of the Funding Transaction (v2 Channel Establishment)

資金調達トランザクションの*期待されるウェイト*は次のように計算されます：
...
非 witness データを 4 倍すると、ウェイトは次のようになります。
...
## コミットメントトランザクションの期待されるウェイト

コミットメントトランザクションの*期待されるウェイト*は次のように計算されます：
...
## HTLC-timeout と HTLC-success トランザクションの期待されるウェイト

HTLC トランザクションの*期待されるウェイト*は次のように計算されます：
...
非 witness データを 4 倍するとウェイトは 376 となり、これに各ケースの witness データ (HTLC-timeout は 285 または 288 + 2、HTLC-success は 324 または 327 + 2) を加えると、ウェイトは次のようになります。
...
以下では:
 ...
 - 秘密鍵は 32 バイトに末尾の 1 を付けた形式で表示します（Bitcoin の「圧縮」秘密鍵の慣習、つまり公開鍵が圧縮されている鍵）。
 - トランザクション署名はすべて RFC6979（HMAC-SHA256 を使用）に基づき決定的です。明示されない限り、有効な署名は対象トランザクションのすべての入力と出力に署名しなければなりません（すなわち `SIGHASH_ALL` [signature hash](https://bitcoin.org/en/glossary/signature-hash) を用いて作成しなければなりません）。なお、クライアントは署名をコンパクトエンコーディングで送信するものであり Bitcoin-script 形式では送信しないため、シグネチャハッシュバイトは送信されません。

資金調達トランザクションの入力は、最初の 2 ブロックを持つテストチェーンを用いて作成されています。2 番目のブロックには利用可能なコインベース出力が含まれます（このような P2PKH 入力は推奨されませんが、[BOLT #2](02-peer-protocol.md#the-funding_created-message) に書かれているように最も簡単な例を提供できます）。
...
資金調達トランザクションは入力 1 つとお釣りの出力を持ちます（この場合、順序は BIP69 によって決まります）。
...
以下では:
 - *local* のトランザクションについて検討します。すなわち、すべての支払いが *local* 側で遅延されます。
 - *local* がオープナーであると仮定します。
 - 秘密鍵は 32 バイトに末尾の 1 を付けた形式で表示します（Bitcoin の「圧縮」秘密鍵の慣習、つまり公開鍵が圧縮されている鍵）。
 - トランザクションの署名はすべて RFC6979（HMAC-SHA256 を使用）に基づき決定的です。

まず、各テストベクトルで共通の基本パラメータを定義します。HTLC は最初の「HTLC のない単純なコミットメントトランザクション」テストでは使用されず、HTLC 5 と 6 は「同じ金額とプリイメージ」テストでのみ使用されます。
...
これらは、任意で実装してよいコンパクトな保存方式を検証します。多くの場合、誤ったエントリは親エントリが明らかになるまで検出できません。エントリは意図的に破損させており、それに伴ってその子エントリもすべて破損しています。

これらのテストでは、シードに `0xFFF...FF`、誤ったエントリに `0x000...00` を用います。
...
アンカー向けのテストベクトルは、付録 C で定義されたテストケースを基にしています。付録 C と同様に、`to_local_msat` と `to_remote_msat` は以下を差し引く前の残高です。
* コミットメント手数料（資金提供者のみ）
* アンカー出力分（資金提供者のみ）
* 進行中の HTLC
...
# Appendix G: デュアルファンディングトランザクションのテストベクトル
...
### 期待されるオープナーの `tx_add_input` (上記の入力 0)：
```
  num_inputs: 1
  tx_add_input:[
    {
...
### 期待されるアクセプターの `tx_add_input` (上記の入力 2)：
```
  num_inputs: 1
  tx_add_input:[
    {
